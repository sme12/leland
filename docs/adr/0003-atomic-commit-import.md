# One atomic `commit_import` tool, not granular per-row writes
...
**Consequence worth flagging:** If the v2 scope grows to non-import use cases ("add a single ad-hoc material", "edit a single past purchase from chat"), those should be **new** tools, not bent out of `commit_import`. The atomic batch shape is specifically tied to the **Import** concept.
