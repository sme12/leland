...
- [ ] P2002 race recovery (Q27): Vitest unit test using a dependency-injected harness that simulates the race by throwing a `P2002` targeting `materials_user_id_name_category_unit_of_measure_key` from an in-flight `material.create`. Asserts the helper returns the `material_conflict` envelope with the now-existing `materialId`, the transaction rolls back atomically (zero purchase rows visible after the throw), and the event is logged at `warn` level with the `clientRequestId`. A real-DB integration test against two concurrent transactions is deferred until shared Postgres test infrastructure exists — out of scope for this issue.
